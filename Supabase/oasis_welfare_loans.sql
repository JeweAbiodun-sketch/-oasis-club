...
begin
  ...
  insert into welfare_loans (
    id, member_id, principal, interest_rate, amount_due, purpose,
    proposed_by, status, due_date
  ) values (
    p_id, p_member_id, p_principal, p_interest_rate,
...
begin
  ...
  set amount_repaid = v_new_total,
      status = case when v_new_total >= amount_due then 'closed' else status end
  where id = p_loan_id;
end;
$$;

grant execute on function is_welfare_committee_pin(text) to anon, authenticated;
grant execute on function propose_welfare_loan(text, text, text, numeric, numeric, text, date) to anon, authenticated;
